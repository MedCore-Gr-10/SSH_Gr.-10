...
export default function DoctorStaffSchedule() {
  ...
  useEffect(() => {
    const loadSchedules = async () => {
      try {
        setLoading(true);
        setError("");
        const [ownScheduleData, hospitalScheduleData] = await Promise.all([
          getStaffSchedules(),
          getDoctorHospitalStaffSchedules(),
        ]);
        setMySchedules(ownScheduleData || []);
        setHospitalSchedules(hospitalScheduleData || []);
      } catch (err) {
  ...
  const ownSchedules = useMemo(
    () =>
      activeHospitalSchedules.filter(
        (schedule) => String(schedule.staff_id) === String(ownStaffId) || ownScheduleIds.has(schedule.id)
      ),
    [activeHospitalSchedules, ownScheduleIds, ownStaffId]
  );
    ...
    <div className="doctor-staff-schedule-page">
      <div className="doctor-staff-schedule-header">
        <div>
          ...
          <p>Check your weekly shifts and see who is working in the hospital by day.</p>
        </div>
      ...
      <section className="doctor-staff-calendar-section">
        <div className="doctor-staff-section-heading">
          ...
          <span>{ownSchedules.length} active shift{ownSchedules.length === 1 ? "" : "s"}</span>
        </div>
      ...
      <section className="doctor-staff-now-section">
        ...
        {workingNowSchedules.length === 0 ? (
          <div className="doctor-staff-now-empty">No active shifts are marked as working right now.</div>
        ) : (
          <div className="doctor-staff-now-grid">
            {workingNowSchedules.map((schedule) => (
              <div key={schedule.id} className="doctor-staff-now-card">
                ...
                <span>{getStaffRole(schedule)} - {getDepartment(schedule)}</span>
                ...
                filteredSchedules.map((schedule) => (
                  <tr key={schedule.id}>
                    <td data-label="Staff member">
                      <strong>{getStaffName(schedule)}</strong>
                      {schedule.staff_id === ownStaffId && <span className="doctor-staff-you">You</span>}
                    </td>
