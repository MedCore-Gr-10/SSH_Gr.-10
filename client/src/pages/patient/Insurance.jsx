...
} from "../../services/patientInsuranceApi";
...
export default function Insurance() {
  ...
  const fetchInsurance = async () => {
    try {
      setLoading(true);
      const data = await getPatientInsurance();
  ...
  const handleSave = async (event) => {
    ...
    try {
      setSaving(true);
      const data = await upsertPatientInsurance(formValues);
      setInsurance(data);
      setShowModal(false);
      setError("");
  ...
  const handleRemoveInsurance = async () => {
    ...
    try {
      setSaving(true);
      await deletePatientInsurance(insurance.id);
      setInsurance(null);
      setFormValues(initialFormValues);
      setError("");
