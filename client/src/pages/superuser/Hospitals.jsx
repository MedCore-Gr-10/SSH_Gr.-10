...
export default function ManageHospitals() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        departments: [...new Set(formData.departments.map(Number))]
      };
      ...
      if (resData.success) {
        ...
        setSearchPersonalNo('');
        setLinkedProfile(null);
        const directorName = linkedProfile
          ? `${linkedProfile.first_name} ${linkedProfile.last_name}`.trim()
          : "the selected director";
        const message = `Hospital "${payload.hospital_name}" was created and linked to ${directorName}.`;
        alert(message);
  ...
  const handleEditFormSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...editFormData,
        departments: [...new Set(editFormData.departments.map(Number))]
      };
      ...
      if (resData.success) {
        ...
        setLinkedProfile(null);
        setSearchPersonalNo('');
        const message = `Hospital "${payload.hospital_name}" was updated successfully.`;
        alert(message);
        setSuccessMessage(message);
        setTimeout(() => setSuccessMessage(''), 5000);
      } else {
  ...
  return (
    <div className="page-container">
      ...
      {successMessage && !isModalOpen && !isDetailsModalOpen && <div className="success-banner">✅ {successMessage}</div>}
